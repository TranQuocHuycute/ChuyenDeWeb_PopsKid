...
public class PopsKidsApplication {
    ...
    CommandLineRunner run(UserService userService, VideoService videoService, CourseService courseService) {
        return args -> {
            ...
            ClassSchedule classSchedule1 =
                    courseService.createClassSchedule(new ClassSchedule(null, 1, 8, "Thứ 7, CN - 15:00", 8, 700000L));

            Rating rating1 = courseService.createRating(new Rating(null, user.getId(), 5, "admin"));
            ...
            courseService.addCourseCatalogsToCourse(course1.getId(), courseCatalog1.getId());
            courseService.addClassSchedulesToCourse(course1.getId(), classSchedule1.getId());
            courseService.addRatingsToCourse(course1.getId(), rating1.getId());
            courseService.addUsersToCourse(course1.getId(), user1.getUsername());

            ClassSchedule classSchedule2 =
                    courseService.createClassSchedule(new ClassSchedule(null, 1, 8, "Thứ 7, CN - 15:00", 8, 700000L));

            Rating rating2 = courseService.createRating(new Rating(null, user.getId(), 5, "admin"));

            CourseCatalog courseCatalog2 =
                    courseService.createCourseCatalog(new CourseCatalog(null,
                            "Ảo thuật",
                            "https://vnw-img-cdn.popsww.com/api/v2/containers/file2/cms_thumbnails/tieng_anh-286b40d22a4d-1656059551172-KyAPqdKG.png?v=0&maxW=160&format=webp",
                            "ảo thật đấy"));
            ...
            courseService.addCourseCatalogsToCourse(course2.getId(), courseCatalog2.getId());
            courseService.addClassSchedulesToCourse(course2.getId(), classSchedule2.getId());
            courseService.addRatingsToCourse(course2.getId(), rating2.getId());
            courseService.addUsersToCourse(course2.getId(), user1.getUsername());
        };
    }
